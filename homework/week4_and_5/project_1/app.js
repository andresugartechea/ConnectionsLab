...
//images of weather reporter
let karen;
let dialogueBox;

//images of cities for the background
let mexico;
let abudhabi;
let newyork;

//images of icon describing the weather
let clear_sky;
let clouds;
let few_clouds;
let rain;
...
//images for animation of the weather
let happy_cloud;
let sad_cloud;
let sun;
let rain_droplets;
...
//this replaces preload()
window.addEventListener("load", function(){
  fetchData();

  //assigning values from the API to the HTML
...
//to change the unit (imperial or metric)
function changeUnit(){
  if (tempUnitValue == 'C'){
    unit = "&units=imperial";
    fetchData();
...
//to display the weather reporter
...
//to display the animation depending on weather
...
//to display dialogue box when keyPressed
function keyPressed(){
   if (dialogue == false){
    dialogue = true;
   }
